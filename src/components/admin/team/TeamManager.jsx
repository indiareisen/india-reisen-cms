      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '16px' }}>
        {team.map(member => (
          <div key={member.id} style={{ backgroundColor: '#fff', padding: '16px', borderRadius: '8px', border: '1px solid #ddd', textAlign: 'center' }}>
            <div style={{ width: '80px', height: '80px', backgroundColor: '#d1356f', borderRadius: '50%', margin: '0 auto 12px', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontSize: '32px' }}>👤</div>
            <h3 style={{ margin: '8px 0', color: '#d1356f' }}>{member.name}</h3>
            <p style={{ margin: '4px 0', color: '#666', fontSize: '12px' }}>{member.role}</p>
            <p style={{ margin: '8px 0', fontSize: '13px' }}>{member.bio}</p>
            <button onClick={() => handleDelete(member.id)} style={{ padding: '6px 12px', backgroundColor: '#ffebee', color: '#c62828', border: '1px solid #ffcdd2', borderRadius: '4px', cursor: 'pointer', fontSize: '12px', width: '100%' }}>Remove</button>
          </div>
        ))}
      </div>
    </div>
